...
import { 
  LayoutDashboard, Users, UserCheck, BookOpen, Hand, GraduationCap, 
...
const SidebarItem = ({ icon: Icon, urdu, english, active, onClick, id }: SidebarItemProps & { id?: string }) => (
  <div 
    id={id}
    onClick={onClick}
  ...
  >
    ...
    <div className="flex flex-col text-right flex-1" dir="rtl">
      <span className={`text-sm font-urdu leading-tight ${active ? 'text-white font-bold' : 'text-white/90'}`}>{urdu}</span>
      <span className={`text-[10px] ${active ? 'text-white/80' : 'text-white/50'}`}>{english}</span>
    </div>
  </div>
);

const GridCard = ({ icon: Icon, title, subtitle, color, onClick, id }: GridCardProps & { id?: string }) => (
  <div 
    id={id}
    onClick={onClick}
    className="card-widget flex flex-col items-center justify-center text-center group"
  >
    <div className={`widget-icon ${color.replace('bg-', 'bg-opacity-10 text-')} group-hover:scale-110 transition-transform`}>
      <Icon className="w-6 h-6" />
    </div>
    <span className="text-sm font-urdu text-slate-800 font-bold mb-1" dir="rtl">{title}</span>
...
export default function Dashboard({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    if (madrassaId) {
      console.log('Registering real-time Firestore synchronization for madrassa ID:', madrassaId);
      unsubscribeFirestore = onSnapshot(doc(db, 'madrassas', madrassaId), (docSnap) => {
        if (docSnap.exists()) {
          ...
          if (fbData && fbData.data) {
            ...
            if (changedAny) {
              console.log('Real-time updates synced from Firestore. Updating local state.');
  ...
  const sidebarItems = [
    ...
    { id: 'academics', path: '/dashboard/grade', icon: GraduationCap, urdu: 'تعلیمی امور', english: 'Academics' },
    { id: 'results', path: '/dashboard/exams', icon: FileText, urdu: 'نتائج', english: 'Exams & Results' },
    { id: 'paper_maker', path: '/dashboard/paper-maker', icon: PenTool, urdu: 'پیپر میکر', english: 'Paper Maker' },
    { id: 'paper_uploader', path: '/dashboard/paper-uploader', icon: Upload, urdu: 'پیپر اپلوڈر', english: 'Paper Uploader' },
    { id: 'paper_checker', path: '/dashboard/paper-checker', icon: CheckSquare, urdu: 'پیپر چیکر', english: 'Paper Checker' },
    { id: 'paper_reports', path: '/dashboard/paper-reports', icon: FileTextIcon, urdu: 'پیپر رپورٹس', english: 'Paper Reports' },
    { id: 'fees', path: '/dashboard/fees', icon: CreditCard, urdu: 'فیس', english: 'Fees' },
    { id: 'staff', path: '/dashboard/staff', icon: UserCog, urdu: 'عملہ و وظائف', english: 'Staff Roles' },
    { id: 'payroll', path: '/dashboard/payroll', icon: Wallet, urdu: 'تنخواہ', english: 'Payroll' },
    { id: 'visitors', path: '/dashboard/visitors', icon: UserPlus, urdu: 'ملاقاتی رجسٹر', english: 'Visitors' },
    { id: 'notifications', path: '/dashboard/messaging', icon: Bell, urdu: 'اعلانات', english: 'Notifications' },
    { id: 'camera', path: '/dashboard/camera', icon: Camera, urdu: 'کیمرہ ویو', english: 'Camera View' },
    { id: 'settings', path: '/dashboard/settings', icon: Settings, urdu: 'ترتیبات', english: 'Settings' },
    { id: 'finance', path: '/dashboard/finance', icon: Landmark, urdu: 'آمد و خرچ', english: 'Finance Management' },
    { id: 'library', path: '/dashboard/library', icon: Library, urdu: 'لائبریری', english: 'Library' },
    { id: 'fatwa', path: '/dashboard/fatwa', icon: Landmark, urdu: 'دارالافتاء', english: 'Dar-ul-Ifta' },
  ...
  const gridCards = [
    ...
    { id: 'scholarship', path: '/dashboard/placeholder', icon: Wallet, title: 'وظیفہ', color: 'bg-emerald-500' },
    { id: 'fees_grid', path: '/dashboard/fees', icon: CreditCard, title: 'فیس', color: 'bg-blue-600' },
    { id: 'grade', path: '/dashboard/grade', icon: Grid, title: 'درجہ', color: 'bg-purple-500' },
    { id: 'paper_maker', path: '/dashboard/paper-maker', icon: PenTool, title: 'پیپر میکر', color: 'bg-indigo-700' },
    { id: 'paper_uploader', path: '/dashboard/paper-uploader', icon: Upload, title: 'پیپر اپلوڈر', color: 'bg-blue-700' },
    { id: 'paper_checker', path: '/dashboard/paper-checker', icon: CheckSquare, title: 'پیپر چیکر', color: 'bg-emerald-700' },
    { id: 'paper_reports', path: '/dashboard/paper-reports', icon: FileTextIcon, title: 'پیپر رپورٹس', color: 'bg-cyan-700' },
    { id: 'staff_grid', path: '/dashboard/staff', icon: UserCog, title: 'معلم و دیگر عملہ', color: 'bg-emerald-600' },
    { id: 'staff_attendance', path: '/dashboard/camera', icon: UserCheck, title: 'حاضری عملہ (QR/Bio)', color: 'bg-lime-500' },
    { id: 'payroll_grid', path: '/dashboard/payroll', icon: Wallet, title: 'تنخواہ', color: 'bg-green-600' },
    { id: 'book', path: '/dashboard/placeholder', icon: Book, title: 'کتاب', color: 'bg-green-700' },
    { id: 'messaging', path: '/dashboard/messaging', icon: MessageSquare, title: 'پیغام رسانی', color: 'bg-blue-500' },
    { id: 'results_grid', path: '/dashboard/exams', icon: FileSearch, title: 'نتائج', color: 'bg-lime-600' },
    { id: 'visitors_grid', path: '/dashboard/visitors', icon: ClipboardList, title: 'ملاقاتیوں کا رجسٹر', color: 'bg-sky-500' },
    { id: 'documents', path: '/dashboard/documents', icon: FileText, title: 'دستاویز کی وصولی', color: 'bg-sky-400' },
    { id: 'complaints', path: '/dashboard/placeholder', icon: Bell, title: 'رجسٹر شکایات', color: 'bg-teal-600' },
    { id: 'fatwa', path: '/dashboard/fatwa', icon: Landmark, title: 'دارالافتاء', color: 'bg-orange-500' },
    { id: 'posts', path: '/dashboard/posts', icon: StickyNote, title: 'جامعہ پوسٹس', color: 'bg-teal-650' },
    { id: 'library', path: '/dashboard/library', icon: Library, title: 'لائبریری', color: 'bg-indigo-500' },
    { id: 'phone_diary', path: '/dashboard/placeholder', icon: Phone, title: 'فون ڈائری', color: 'bg-blue-700' },
    { id: 'notes', path: '/dashboard/placeholder', icon: StickyNote, title: 'یادداشت', color: 'bg-indigo-800' },
    { id: 'finance', path: '/dashboard/finance', icon: Calculator, title: 'مالیاتی حساب', color: 'bg-red-500' },
  ...
  return (
    <div className="flex flex-row-reverse h-screen bg-[#F4F7F6] overflow-hidden print:block print:h-auto print:bg-transparent relative">
      ...
      {/* Sidebar */}
      <div className={`${isSidebarOpen ? 'w-64' : 'w-0'} sidebar-gradient flex flex-col transition-all duration-300 print:hidden overflow-y-auto overflow-x-hidden custom-scrollbar shadow-2xl`}>
        <div className="p-8 mb-4 min-w-[256px]">
          <div className="flex flex-col items-center gap-4 mb-2">
            ...
            <div className="flex flex-col text-center">
              <span className="text-[10px] font-bold text-white/50 uppercase tracking-[0.2em]">Management System</span>
              <span className="text-sm font-urdu font-bold text-white mt-1">{systemSettings.jamiaName}</span>
            </div>
          </div>
          ...
          <div className="bg-white/10 rounded-2xl p-4 border border-white/10 backdrop-blur-sm">
            <div className="flex items-center gap-3 mb-3" dir="rtl">
              ...
              <div className="flex flex-col text-right">
                <span className="text-xs font-bold text-white font-urdu">عبدالرحمان حبیب</span>
                <span className="text-[9px] text-white/50 uppercase tracking-wider">Super Admin</span>
              </div>
            </div>
            <button 
      ...
      <div className="flex-1 flex flex-col overflow-hidden print:overflow-visible print:h-auto">
        ...
        <Routes>
          ...
          <Route path="/camera" element={<CameraView onBack={() => navigate('/dashboard')} />} />
          <Route path="/finance" element={<FinanceManagement onBack={() => navigate('/dashboard')} />} />
          <Route path="/staff" element={<StaffManagement onBack={() => navigate('/dashboard')} />} />
          <Route path="/messaging" element={<MessagingCenter onBack={() => navigate('/dashboard')} />} />
          <Route path="/grade" element={hasPermission('academics') ? <GradeManagement onBack={() => navigate('/dashboard')} /> : <Navigate to="/dashboard" replace />} />
          <Route path="/paper-maker" element={hasPermission('exams') ? <PaperMaker onBack={() => navigate('/dashboard')} /> : <Navigate to="/dashboard" replace />} />
          <Route path="/paper-uploader" element={hasPermission('paper_uploader') ? <PaperUploader onBack={() => navigate('/dashboard')} /> : <Navigate to="/dashboard" replace />} />
          <Route path="/paper-checker" element={hasPermission('paper_checker') ? <PaperChecker onBack={() => navigate('/dashboard')} /> : <Navigate to="/dashboard" replace />} />
          <Route path="/paper-reports" element={hasPermission('paper_reports') ? <PaperReports onBack={() => navigate('/dashboard')} /> : <Navigate to="/dashboard" replace />} />
          <Route path="/library" element={<BookLibrary onBack={() => navigate('/dashboard')} />} />
          <Route path="/reports" element={<ReportsView onBack={() => navigate('/dashboard')} />} />
          <Route path="/fatwa" element={<DarulIfta onBack={() => navigate('/dashboard')} />} />
          <Route path="/posts" element={<JamiaPosts onBack={() => navigate('/dashboard')} />} />
          <Route path="/payroll" element={<PayrollManagement onBack={() => navigate('/dashboard')} />} />
          <Route path="/fees" element={<FeesManagement onBack={() => navigate('/dashboard')} />} />
          <Route path="/documents" element={<DocumentManagement onBack={() => navigate('/dashboard')} />} />
          <Route path="/recycle-bin" element={<RecycleBin onBack={() => navigate('/dashboard')} />} />
          <Route path="/placeholder" element={<ModulePlaceholder title={activeModuleName} onBack={() => navigate('/dashboard')} />} />
            ...
            <>
              ...
              <header className="h-20 bg-white flex items-center justify-between px-8 border-b border-slate-100 shadow-sm">
                  ...
                  <div className="flex items-center gap-3">
                    <button 
                      ...
                      className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                        isSyncing ? 'bg-blue-600 text-white animate-pulse' : 'bg-blue-50 text-blue-600 hover:bg-blue-100 border border-blue-100'
                      }`}
                    >
                      <RefreshCw className={`w-4 h-4 ${isSyncing ? 'animate-spin' : ''}`} />
                      <span className="font-urdu">{isSyncing ? 'سنک ہو رہا ہے...' : 'ڈیٹا سنک کریں'}</span>
                    </button>
                ...
                <div className="flex items-center gap-6" dir="rtl">
                  <div className="flex flex-col text-right">
                    <h2 className="text-slate-900 text-xl font-urdu font-bold">نظام برائے انتظام مدرسہ</h2>
                    <p className="text-slate-400 text-[11px] font-urdu font-medium tracking-wide">{systemSettings.jamiaName} - پورٹل</p>
                  </div>
              ...
              <div className="bg-white px-8 py-2 flex justify-end gap-10 border-b border-slate-100" dir="rtl">
                {['مرکز', 'اعداد و شمار', 'متفرق', 'رپورٹس'].map((tab) => {
                  const isActive = (tab === 'رپورٹس' && location.pathname === '/dashboard/reports');
                  return (
                    <button 
                      key={tab} 
                      onClick={() => {
                        if (tab === 'رپورٹس') navigate('/dashboard/reports');
                        else { setActiveModuleName(tab); navigate('/dashboard/placeholder'); }
                      }}
              ...
              <main className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8" dir="rtl">
                   <div className="card-widget border-r-4 border-r-blue-600">
                      <div className="flex justify-between items-start">
                         <div>
                            ...
                            <h4 className="text-2xl font-bold text-slate-800 mt-1">1,240</h4>
                         </div>
                   ...
                   <div className="card-widget border-r-4 border-r-emerald-600">
                      <div className="flex justify-between items-start">
                         <div>
                            <span className="text-[10px] text-slate-400 font-bold uppercase">Daily Attendance</span>
                            <h4 className="text-2xl font-bold text-slate-800 mt-1">98%</h4>
                         </div>
                         <div className="bg-emerald-50 p-2 rounded-lg text-emerald-600"><UserCheck className="w-5 h-5" /></div>
                      </div>
                      <div className="mt-4 flex items-center gap-2 text-green-500 text-[10px] font-bold">
                   ...
                   <div className="card-widget border-r-4 border-r-orange-600">
                      <div className="flex justify-between items-start">
                         <div>
                            <span className="text-[10px] text-slate-400 font-bold uppercase">Pending Fees</span>
                            <h4 className="text-2xl font-bold text-slate-800 mt-1">Rs. 45k</h4>
                         </div>
                         <div className="bg-orange-50 p-2 rounded-lg text-orange-600"><Wallet className="w-5 h-5" /></div>
                      </div>
              ...
              <footer className="h-16 bg-white border-t border-slate-100 px-8 flex items-center justify-between text-[10px] text-slate-400">
                <div className="flex flex-col">
                  <span className="font-urdu text-left font-bold text-slate-500">تمام حقوق محفوظ ہیں۔ ڈیزائن و ڈویلپمنٹ: عبدالرحمن حبیب</span>
                  <span className="uppercase tracking-widest text-left">© 2026 {systemSettings.jamiaName} | Professional Portal V3</span>
                </div>
                <div className="flex gap-6 items-center">
                   <div className="flex flex-col items-end">
                      <span className={`flex items-center gap-1 text-[9px] font-bold ${isSyncing ? 'text-blue-500' : 'text-emerald-500'}`}>
                        <div className={`w-1.5 h-1.5 rounded-full ${isSyncing ? 'bg-blue-500 animate-ping' : 'bg-emerald-500'}`} /> 
                        {isSyncing ? 'Syncing...' : 'System Online'}
                      </span>
                   </div>
                   <div className="h-6 w-px bg-slate-100" />
                   <span className="px-3 py-1 bg-slate-50 text-slate-500 rounded-lg border border-slate-200">Stable Version</span>
                </div>
              </footer>
            </>
          } />
